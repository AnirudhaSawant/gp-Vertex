...
public class FirstHttpServer extends AbstractVerticle {
    ...
    public void start() throws Exception {
        ...
        List<Book> books = Arrays.asList(
                ...
                new Book(1,"prog in java",850,950),
                new Book(1,"scala programming",920,1500)
        );

        Router router = Router.router(vertx);
        router.get("/books").handler(routingContext -> {
            JsonArray data = new JsonArray(books);
            routingContext.response().putHeader("Content-Type","application/json").end(data.encode());
        });
    ...
    public static void main(String[] args) {
        ...
        DeploymentOptions options = new DeploymentOptions().setConfig(new JsonObject().put("port",8083));
